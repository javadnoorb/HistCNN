...
if [ ! -e 00-app-config.sh ]; then
  ...
  echo -e "export gcs_ann_path='gs://histology-cnn/data/pancancer_annotations/her2_cache_anns'" >> 00-app-config.sh
  echo -e "export temp_path='$(mktemp -d --suffix .histcnn.her2)' # a temporary path to be used for different purposes" >> 00-app-config.sh
  return
fi
