...
if [ ! -e 00-app-config.sh ]; then
  ...
  job_tag="tfrecords_run_tn_crossclassify"
  ...
  echo -e "export max_replica=405  # will be used for the max replica to be launched in deplyoment" >> 00-app-config.sh
  echo -e "export ack_deadline_seconds=60  # Pub/Sub deadline from 0 to 600" >> 00-app-config.sh
  echo -e "export annotations_path='data/pancancer_annotations/tn_frozen_cache_anns' #path of annotation files" >> 00-app-config.sh
  echo -e "export results_path='data/run-results/frozen_undersampled/' # where to save the outputs of the run" >> 00-app-config.sh
  echo -e "export pancancer_tfrecords_path='tfrecords/frozen/tn' # tfrecords path" >> 00-app-config.sh
  echo -e "export gcs_output_path='gs://histology-cnn/data/tn/cross-classification-frozen/include-training/' # GCS path for saving output jsons that contain AUCs" >> 00-app-config.sh  
