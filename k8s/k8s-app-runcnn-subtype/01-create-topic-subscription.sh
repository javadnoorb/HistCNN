...
if [ ! -e 00-app-config.sh ]; then
  ...
  job_tag="tfrecords_runcnn_subtype"
  ...
  echo -e "export max_replica=15  # will be used for the max replica to be launched in deplyoment" >> 00-app-config.sh
  echo -e "export ack_deadline_seconds=60  # Pub/Sub deadline from 0 to 600" >> 00-app-config.sh
  echo -e "export annotations_path='data/pancancer_annotations/subtypes_annotations' #path of annotation files" >> 00-app-config.sh
  echo -e "export results_path='data/run-results/undersampled_subtype_with_relu/' # where to save the outputs of the run" >> 00-app-config.sh
  echo -e "export pancancer_tfrecords_path='tfrecords/cache_subtypes' # tfrecords path" >> 00-app-config.sh
return
fi
