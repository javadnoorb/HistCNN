...
if [ ! -e 00-app-config.sh ]; then
  ...
  echo -e "export task_kind='Task:Tiling'  # Task type for DataStore" >> 00-app-config.sh
  echo -e "export topic_name='slide_tiling'  # Topic for Pub/Sub" >> 00-app-config.sh
  echo -e "export subscription_name='slide_tiling_sub'  # Subscription for Pub/Sub" >> 00-app-config.sh
  echo -e "export image_name='tiling'  # Image name used for docker and Container Registry" >> 00-app-config.sh
  echo -e "export tile_size=512  # size of tiles to be created: tile_size x tile_size" >> 00-app-config.sh
  echo -e "export deployment_name='tile-slides'  # will be used for deployment, container, and pod." >> 00-app-config.sh
